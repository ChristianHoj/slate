...
if(defined('QUALIFICATION_2015_REQUIRE_DETAIL') || isset($_GET['2015_qualification_detail'])) {
...
if(!$nOK || $bDoCreateDetailTable || isset($_GET['force_create'])) {
	$bDoCreateTable = true;
...
/*var_dump($bDoCreateTable);
var_dump($bDoCreateDetailTable);
*/
...
*/
		...
		while($aRow = mysql_fetch_assoc($oSubRS)) {
			if($nTheMedlid != $aRow['medlid']) {
				$aRow['meeting_points'] = $nTheOrgPoints = 1;
				$nTheMedlid = $aRow['medlid'];
			} else {
				switch($nTheOrgPoints + 1) {
					case 2 :		$aRow['meeting_points'] = $nTheOrgPoints = 2; break;
					case 3 :		$aRow['meeting_points'] = $nTheOrgPoints = 3; break;
					case 4 :		$aRow['meeting_points'] = $nTheOrgPoints = 4; break;
					case 5 :		$aRow['meeting_points'] = $nTheOrgPoints = 5; break;
					case 6 :		$aRow['meeting_points'] = $nTheOrgPoints = 6; break;				
				}
				reset($aRow);
...
// 2015-09-04 STILL OK
	...
	if(!empty($a2015QRules['scp_sponsor'])) {  // START [HUGE-IF-001]
			...
			$sQuery = sprintf(' CREATE TEMPORARY TABLE %s AS  ' ,$sSponsorTable);
			...
			$sQuery = sprintf(' CREATE TEMPORARY TABLE %s AS SELECT da_reference.* , "da_reference" source FROM da_reference ' , $sDaAllTableName);
			$aLogs[] = '<i>' . $sQuery . '</i>';
			mysql_query($sQuery) or die(mysql_error() . ' line ' . __LINE__);
			...
			$sQuery = ' CREATE TEMPORARY TABLE tmp_sponsor_final_2015 AS ';
		...
		// 	ORG $sQuery .= sprintf(' SELECT "adjusted_sponsor_point" , sponsor_id ,  IF( LEAST(customers, %d - SUM(deducted) ) <0 , 0 , LEAST(customers, %d - SUM(deducted) )) sponsor_point , DATE("0000-00-00") , DATE("0000-00-00")  FROM  %s t LEFT JOIN %s da ON t.medlid = da.Reference WHERE sponsor_id IS NOT NULL GROUP BY sponsor_id ' , $nTogetherSponsorPoints , $nTogetherSponsorPoints ,$sSponsorTable , $sDaAllTableName);
			...
			if($bDoCreateDetailTable) { 
				...
				*/
				...
				mysql_query($sSubQuery) or die(mysql_error() . ' line ' . __LINE__);
		...
		//		
			...
			//	$sSubQuery .=  sprintf(' IF( LEAST(customers, %d - SUM(deducted) ) <0 , 0 , LEAST(customers, %d - SUM(deducted) )) sponsor_point  ' ,  )
				...
				$aLogs[] = ' sub_detail ';	
				$aLogs[] = $sSubQuery;
				mysql_query($sSubQuery) or die(mysql_error() . ' <br>' . $sSubQuery . ' <br /> line ' . __LINE__);
				...
				$aLogs[] = $sSubQuery;
				$oRS = mysql_query($sSubQuery) or die(mysql_error() . ' line ' . __LINE__);
				$aLogs[] = sprintf(' D) => %d rows selected ', mysql_num_rows($oRS));
				$aLogs[] = '============================================';
...
*/
	...
	$sQuery = sprintf(' SELECT "webpakkes" code , medlid , MAX(name) name , COUNT(webpakker)webpakker , SUM(  IF( DATE(webpakkeDate) >= "%s" , 5 , 3 ))  webpakker_points , MAX(webpakkeDate) last_webpakkers  ' , $sAugustNewRuleFrom)
					. ' 	FROM TEMP_spw  GROUP BY medlid  ';
