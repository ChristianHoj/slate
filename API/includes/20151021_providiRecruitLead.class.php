...
class ProvidiRecruitLead extends ProvidiObject{
	...
	static function tidyUpEmnerRow($oEmneIdealvaegtRow) {
...
class ProvidiRecruitLeadList {
	...
	function getList($sOwner , $sFromDate=null , $sToDate=null , $aMode=null , $nLimit = 200) {
		...
		if(!empty($sFromDate) && !empty($sToDate)) {		
			...
			$aWhere['between_date'] = sprintf(' DATE(assigned_on) BETWEEN "%s" AND "%s" ',  $oDB->esc($sFromDate),  $oDB->esc($sToDate));
		} else {
			if(!empty($sFromDate)) {			
				$sFromDate = providiDateTime($sFromDate , 'SQL');
				$aWhere['from_date'] = sprintf(' DATE(assigned_on) >= "%s" ' , $oDB->esc($sFromDate));
			}
			if(!empty($sToDate)) {			
				$sToDate = providiDateTime($sToDate , 'SQL');
				$aWhere['to_date'] = sprintf(' DATE(assigned_on) <= "%s" ' , $oDB->esc($sToDate));
			}		
		}
